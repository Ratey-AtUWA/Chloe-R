# 1. RUN THIS FORST SECTION BEFORE THE CODE UNDER HEADING 2. BELOW
# read data
...
with(device125, plot(VOC ~ dt, pch=20, cex=0.5, log="y",
     xlab="Date/time"))
with(device125byday, lines(VOC ~ date, col="coral", lwd=3, type="o"))
with(device125byhour, lines(VOC ~ date, col="#0080ff80",
                           cex=0.85, pch=16, lwd=2, type="o"))
legend("topright", bty="n", cex=1.2, inset=c(0.02,0),
       title=expression(bold("device125")),
